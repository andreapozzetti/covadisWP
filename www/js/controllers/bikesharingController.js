/* BIKESHARING CONTROLLER */

angular.module('BikesharingCtrl', [])
.controller('bikesharingCtrl', function($scope, $routeParams, $window, $location, parking, bikesharing, androidNavigate, push, goBack) {
  ...
  $scope.bikesharingInfo = bikesharing.getBikesharing($routeParams.idBikesharing).then(function(data) {
    ...
    var bikesharingData = {
                        idBikesharing: data.idBikesharing,
                        name: data.name,
    ...
    $scope.bikesharingInfo = bikesharingData;
    
    return $scope.bikesharingInfo;
  
  });

  $scope.navigate = function(idBikesharing,bikesharingName,bikesharingLatitude,bikesharingLongitude){

    localStorage.setItem("idBikesharing", idBikesharing);
    localStorage.setItem("bikesharingName", bikesharingName);

    $scope.coordinates = ""+parseFloat(bikesharingLatitude).toFixed(5)+","+parseFloat(bikesharingLongitude).toFixed(5)+"";
